...
class HomResultController: UIViewController {
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        delegate?.homResultControllerSizeChange(viewSize: view.frame.width)
    }
...
extension HomResultController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 0 && homBeamResult != nil {
            ...
            cell.sixCellWidth = (view.frame.width - 64 - 6 * 7) / 6.0
            cell.fourCellWidth = (view.frame.width - 64 - 6 * 5) / 4.0
            delegate = cell
            return cell
