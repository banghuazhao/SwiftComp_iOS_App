...
extension UIButton {
    ...
    func methodButtonDesign(under: UIView) {
        ...
        testLabel.anchor(top: self.topAnchor, leading: nil, bottom: self.bottomAnchor, trailing: self.trailingAnchor, padding: UIEdgeInsets(top: 0, left: 0, bottom: 0, right: -8))
        testLabel.font = UIFont.fontAwesome(ofSize: 15, style: .solid)
        testLabel.text = String.fontAwesomeIcon(name: .chevronCircleDown)
        testLabel.textColor = self.tintColor
    ...
    func cloudCalculateButtonDesign(vc: UIViewController, calculateFunction: Selector) {
        ...
        self.setAttributedTitle(attributedStr, for: .normal)
        self.layer.cornerRadius = self.intrinsicContentSize.height / 2.5
        self.backgroundColor = .swiftcompLogoColor
    ...
    func swiftCompOutputButtonDesign(under: UIView) {
        ...
        self.setTitle("Show SwiftComp Calculation Info", for: UIControl.State.normal)
        self.frame = CGRect(x: 0, y: 0, width: 280, height: 32)
